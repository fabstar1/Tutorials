# HTML Grundlagen

HTML (HyperText Markup Language) ist die Sprache, mit der du den **Inhalt** einer Webseite beschreibst. Jedes Element wird mit sogenannten **Tags** ausgezeichnet.

## Themen

- [Grundstruktur](./01-grundstruktur.md)
- [Textelemente](./02-textelemente.md)
- [Listen & Tabellen](./03-listen-tabellen.md)
- [Formulare](./04-formulare.md)
- [Bilder & Links](./05-bilder-links.md)

> **Das ist wichtig weil...** Blazor-Komponenten basieren auf HTML. Ohne HTML-Kenntnisse kannst du keine sinnvollen Benutzeroberflächen bauen.
